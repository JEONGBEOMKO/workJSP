...
<%
  ...
  PST=CN.prepareStatement(msg);  //Statement 명령어 아님
  ...
  if(Gtotal>0){
	  //jsp에서 쿠키는 내장클래스, 쿠키등록 response
  	 session.setAttribute("naver", usera); //userid값을 가짜변수 세션naver가짜이름에 넘김  --세션
  	 //naver가 전역변수의 여할을 해주는. 
  	 // 이 밑이 쿠키 부분.. jsp에서 쿠키는 내장클래스, 쿠키등록 response
  	 request.setAttribute("idx", usera);  //session 및 request도 값전달
  	 // 값전달 a캐그
  	 Cookie ck = new Cookie("google", usera); //웹서버에서 제공되는 쿠키내장클래스 객체화
  	 //쿠키는 사용자 컴퓨터
